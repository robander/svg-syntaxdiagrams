...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:function name="f:degree-to-radian" as="xs:float" use-when="function-available('math:constant')">
        ...
        <xsl:sequence select="xs:float($x * math:constant('PI', 10) div 180.0)"/>
    </xsl:function>
    
    <xsl:function name="f:cos" as="xs:float" use-when="function-available('math:cos')">
        <xsl:param name="x" as="xs:float"/>
        <xsl:sequence select="xs:float(math:cos(f:degree-to-radian($x)))"/>
    </xsl:function>

    <xsl:function name="f:sin" as="xs:float" use-when="function-available('math:sin')">
        <xsl:param name="x" as="xs:float"/>
        <xsl:sequence select="xs:float(math:sin(f:degree-to-radian($x)))"/>
    </xsl:function>

    <xsl:function name="f:tan" as="xs:float" use-when="function-available('math:tan')">
        <xsl:param name="x" as="xs:float"/>
        <xsl:sequence select="xs:float(math:tan(f:degree-to-radian($x)))"/>
    </xsl:function>
        ...
        <xsl:variable name="width" as="xs:float">
            <xsl:choose>
                <xsl:when test="matches(@width, '^.*px$')">
                    <xsl:sequence select="xs:float(number(substring-before(@width, 'px')))"/>
                </xsl:when>
                <xsl:when test="matches(@width, '^.*in$')">
                    <xsl:sequence select="xs:float(number(substring-before(@width, 'in'))*$xresolution)"/>
                </xsl:when>
                <xsl:when test="matches(@width, '^.*mm$')">
                    <xsl:sequence select="xs:float(number(substring-before(@width, 'in'))*$xresolution*25.4)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:message terminate="yes">
        ...
        <xsl:variable name="height" as="xs:float">
            <xsl:choose>
                <xsl:when test="matches(@height, '^.*px$')">
                    <xsl:sequence select="xs:float(number(substring-before(@height, 'px')))"/>
                </xsl:when>
                <xsl:when test="matches(@height, '^.*in$')">
                    <xsl:sequence select="xs:float(number(substring-before(@height, 'in'))*$xresolution)"/>
                </xsl:when>
                <xsl:when test="matches(@height, '^.*mm$')">
                    <xsl:sequence select="xs:float(number(substring-before(@height, 'in'))*$xresolution*25.4)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:message terminate="yes">
